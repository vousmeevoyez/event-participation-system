
				<!-- Wrapper -->
					<section id="wrapper">
										<header>
											<div class="inner">
												<h2 class="competition-heading">Futsal COMPETITION</h2>
													<p class="competition-desc">Kegiatan untuk menyalurkan minat dan bakat di bidang Olahraga (Futsal) dengan tujuan untuk meningkatkan jiwa sporvitas dan mempererat tali silaturahmi melalui kegiatan Olahraga
													</p>
												<a href="<?=base_url('/pages/download/hifest_rulebook.pdf');?>"><h3>Download Rulebook</h3></a>
											</div>
										</header>

						<!-- Content -->
							<div class="wrapper">
								<div class="inner">
									<section>
										<h2 class="comp-heading">PRIZE</h2>
											<div class="box alt">
													<div class="row uniform prize-row">
														<div class="4u"><span class="image fit"><img src="<?=base_url('assets/pages/img/trophy.png');?>" alt="" class="prize"/></span>
															<h1>Juara 1</h1>
														</div>
														<div class="4u"><span class="image fit"><img src="<?=base_url('assets/pages/img/trophy2.png');?>" alt="" class="prize" /></span>
															<h1>Juara 2</h1>
														</div>
														<div class="4u"><span class="image fit"><img src="<?=base_url('assets/pages/img/trophy3.png');?>" alt="" class="prize" /></span>
															<h1>Juara 3</h1>
														</div>
													</div>
													<p style="text-align: center;"><sub>*Hadiah dapat berubah sewaktu-waktu sesuai jumlah peserta yang akan berpartisipasi</sub></p>
													<hr>
													<!--<div class="row uniform prize-row">
														<div class="4u"><span class="image fit"><img src="<?=base_url('assets/pages/img/trophy2.png');?>" alt="" class="prize" /></span>
														</div>
														<div class="4u">
															<h1>2ND PLACE</h1>
														</div>
													</div>
													<hr>
													<div class="row uniform prize-row">
														<div class="4u"><span class="image fit"><img src="<?=base_url('assets/pages/img/trophy3.png');?>" alt="" class="prize" /></span>
														</div>
														<div class="4u">
															<h1>3RD PLACE</h1>
														</div>
													</div>
													<hr>-->
											</div>
									</section>
									<br>
									<section>
										<h2 class="comp-heading">TIMELINE</h2>
											<div class="table-wrapper">
												<table>
													<tbody>
														<tr>
															<td>Pendaftaran</td>
															<td>1 Agustus - 1 Oktober 2017.</td>
														</tr>
														<tr>
															<td>Pelaksanaan</td>
															<td>6 November 2017.</td>
														</tr>
													</tbody>
													<tfoot>
														<tr>
														</tr>
													</tfoot>
												</table>
											</div>
									</section>
									<section>
										<a href="<?=base_url('participant/signup');?>" class="button special fit">Daftar Sekarang</a>
									</section>
										
									</section>
